 ...
 */
define( 'FLASH_CARD_VERSION', '29.6.57' );
...
function custom_portfolio_sets_columns($columns) {
    $columns['custom_column'] = 'Featured';
    return $columns;
}
add_filter('manage_portfolio_sets_posts_columns', 'custom_portfolio_sets_columns',20);

// Populate the custom column with data
function custom_portfolio_sets_column_content($column, $post_id) {
    if ($column === 'custom_column') {
       ?>
	   <span class='featured-set-checkbox'><input class='star' type='checkbox'></span>
	   <?php
    }
}
add_action('manage_portfolio_sets_posts_custom_column', 'custom_portfolio_sets_column_content', 20, 2);
